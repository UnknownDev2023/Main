...
const DisplayVaults: React.FC<{ vaults: Vault[]; openId?: number }> = ({ vaults, openId }) => {
  ...
  const vaultsListView = vaults.map((vault) => {
    ...
    const liquidityUrl = `https://cosmicx.finance/swap/`
    ...
    ${vault?.version === VaultVersion.V1 ? 'COSMIC' : ' LP'}`
          ...
          <Flex sx={styles.cardContent}>
            <Flex sx={{ ...styles.onlyDesktop, maxWidth: '140px', width: '100%' }}>
              <ListViewContent
                ...
                toolTip={t(
                  'Daily APY includes COSMIC rewards (calculated based on token value, reward rate, and percentage of vault owned) and DEX swap fees, compounded daily.',
                )}
            ...
            <ListViewContent
              ...
              toolTip={t(
                'Annual APY includes annualized COSMIC rewards (calculated based on token value, reward rate, and percentage of vault owned) and DEX swap fees, compounded daily.',
              )}
              toolTipPlacement="bottomLeft"
