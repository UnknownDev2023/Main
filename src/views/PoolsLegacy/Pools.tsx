...
const StyledLabel = styled.div<LabelProps>`
  ...
  background-color: ${({ active }) => active && '#3c817f'};
`
...
const Pools: React.FC = () => {
  ...
  const gnanaOnlyPools = openPools.filter((pool) => pool.stakingToken?.symbol === 'COSMICX')
  const bananaOnlyPools = openPools.filter((pool) => pool.stakingToken?.symbol === 'COSMIC')

  const gnanaInactivePools = finishedPools.filter((pool) => pool.stakingToken?.symbol === 'COSMICX')
  const bananaInactivePools = finishedPools.filter((pool) => pool.stakingToken?.symbol === 'COSMIC')
  const gnanaStakedOnlyPools = openPools.filter(
    (pool) =>
      pool.userData &&
      new BigNumber(pool.userData.stakedBalance).isGreaterThan(0) &&
      pool.stakingToken?.symbol === 'COSMICX',
  )
  const bananaStakedOnlyPools = openPools.filter(
    (pool) =>
      pool.userData &&
      new BigNumber(pool.userData.stakedBalance).isGreaterThan(0) &&
      pool.stakingToken?.symbol === 'COSMIC',
  )
  ...
  const allStakedOnlyPools = openPools.filter(
    (pool) =>
      pool.userData &&
      new BigNumber(pool.userData.stakedBalance).isGreaterThan(0) &&
      (pool.stakingToken?.symbol === 'COSMIC' || pool.stakingToken?.symbol === 'COSMICX'),
  )
